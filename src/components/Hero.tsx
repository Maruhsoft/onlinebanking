        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              <Link 
                to={siteConfig.urls.openAccount}
      ...
      <div className="relative border-t border-blue-800 bg-blue-900 bg-opacity-50">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {Object.entries(siteConfig.images.partners).map(([name, src]) => (
              <div key={name} className="flex items-center justify-center">
                <img 
                  src={src} 
                  alt={name.replace(/([A-Z])/g, ' $1').trim()} 
                  className="h-8 opacity-75 object-contain"
                />
              </div>
            ))}
          </div>
